...
{
    m_ws_programDirPath.clear();
    m_ws_programFilename.clear();
    initFilePath("");
    EXPECT_EQ(m_ws_programDirPath.empty(), false);
    EXPECT_EQ(m_ws_programDirPath, sofa::helper::Utils::getSofaUserLocalDirectory());
    EXPECT_EQ(m_ws_programFilename.empty(), false);
    EXPECT_EQ(m_ws_programFilename, "output" + m_program.getExtension());
}
...
void TestProgramWindow::testInitFilePath_pathDoesNotExist()
{
    ...
    EXPECT_EQ(m_ws_programDirPath, sofa::helper::Utils::getSofaUserLocalDirectory());
    EXPECT_EQ(m_ws_programFilename.empty(), false);
    EXPECT_EQ(m_ws_programFilename, "file" + m_program.getExtension());
}
...
void TestProgramWindow::testInitFilePath_file()
{
    ...
    EXPECT_EQ(m_ws_programDirPath.empty(), false);
    EXPECT_NE(m_ws_programDirPath, sofa::helper::Utils::getSofaUserLocalDirectory());
    EXPECT_EQ(m_ws_programFilename.empty(), false);
    EXPECT_EQ(m_ws_programFilename, "file" + m_program.getExtension());
}
...
TEST_F(TestProgramWindow, testEmptyProgramDirPath) { EXPECT_EQ(m_ws_programDirPath.empty(), true); }
TEST_F(TestProgramWindow, testEmptyProgramFilename) { EXPECT_EQ(m_ws_programFilename.empty(), true); }
